...
CREATE NONCLUSTERED INDEX [IDX_tblWorkContent_StatusFields]
    ON [dbo].[tblWorkContent]([ReadyToPublish] ASC, [DelayedPublish] ASC, [HasBeenPublished] ASC);
...
GO
PRINT N'Creating DF__tblContentLanguage__PendingPublish...';


GO
ALTER TABLE [dbo].[tblContentLanguage]
    ADD CONSTRAINT [DF__tblContentLanguage__PendingPublish] DEFAULT (0) FOR [PendingPublish];
...
GO
PRINT N'Creating DF__tblWorkPa__Ready__4D7F7902...';


GO
ALTER TABLE [dbo].[tblWorkContent]
    ADD CONSTRAINT [DF__tblWorkPa__Ready__4D7F7902] DEFAULT (0) FOR [ReadyToPublish];
...
GO
PRINT N'Creating DF__tblWorkPa__HasBe__4F67C174...';


GO
ALTER TABLE [dbo].[tblWorkContent]
    ADD CONSTRAINT [DF__tblWorkPa__HasBe__4F67C174] DEFAULT (0) FOR [HasBeenPublished];


GO
PRINT N'Creating DF__tblWorkPa__Rejec__505BE5AD...';


GO
ALTER TABLE [dbo].[tblWorkContent]
    ADD CONSTRAINT [DF__tblWorkPa__Rejec__505BE5AD] DEFAULT (0) FOR [Rejected];


GO
PRINT N'Creating DF__tblWorkPa_Delayed...';


GO
ALTER TABLE [dbo].[tblWorkContent]
    ADD CONSTRAINT [DF__tblWorkPa_Delayed] DEFAULT (0) FOR [DelayedPublish];
...
GO
PRINT N'Creating DF__tblContent__Pending__31D75E8D...';


GO
ALTER TABLE [dbo].[tblContent]
    ADD CONSTRAINT [DF__tblContent__Pending__31D75E8D] DEFAULT (0) FOR [PendingPublish];
...
END
GO
PRINT N'Creating [dbo].[netPageListPaged]...';
...
END
GO
PRINT N'Creating [dbo].[netPageDataLoad]...';
...
END
GO
PRINT N'Creating [dbo].[netMasterLicenseSet]...';
...
END
GO
PRINT N'Creating [dbo].[netMasterLicenseGet]...';
...
BEGIN
	...
	SELECT TOP 1 P.pkID as ID, P.fkContentTypeID as ContentTypeID, P.fkParentID as ParentID, P.ContentGUID, PL.LinkURL, P.Deleted, P.PendingPublish, PL.Created, PL.Changed, PL.Saved, PL.StartPublish, PL.StopPublish, P.ExternalFolderID, P.ContentAssetsID, P.fkMasterLanguageBranchID as MasterLanguageBranchID, PL.ContentLinkGUID as ContentLinkID, PL.AutomaticLink, PL.FetchData, P.ContentType
...
BEGIN
	...
	INSERT INTO tblContent 
		(fkContentTypeID, CreatorName, fkParentID, ExternalFolderID, ContentAssetsID, ContentOwnerID, PendingPublish, ContentGUID, ContentPath, ContentType, Deleted)
	VALUES
		(@ContentTypeID, @UserName, @ParentID, @FolderID, @ContentAssetsID, @ContentOwnerID, 1, @ContentGUID, @Path, @ContentType, @Delete)
...
BEGIN
   SET NOCOUNT ON
	...
	if (@Force = 1 OR EXISTS(SELECT * FROM tblWorkContent    WITH(NOLOCK) WHERE fkContentID = @ContentLink AND fkLanguageBranchID = @LangID AND HasBeenPublished = 1 AND CommonDraft = 1))
	BEGIN 	
...
BEGIN
	...
	END			
		...
		IF EXISTS(SELECT * FROM tblContentLanguage WHERE fkContentID=@ContentID AND fkLanguageBranchID=@CurrentLangBranchID AND PendingPublish=1)
		BEGIN
...
END
GO
PRINT N'Creating [dbo].[editRejectCheckedInContentVersion]...';
...
END
GO
PRINT N'Creating [dbo].[editCheckOutContentVersion]...';
...
END
GO
PRINT N'Creating [dbo].[editCheckInContentVersion]...';
...
END
GO
PRINT N'Creating [dbo].[netPageDataLoadVersion]...';
...
BEGIN
	...
	IF (@MaxVersions IS NULL OR @MaxVersions=0)
		...
		BEGIN
			...
			SELECT @PublishedVersion = PublishedVersion FROM tblContentLanguage WHERE fkContentID=@ContentID AND fkLanguageBranchID=@CurrentLanguage
			SELECT @ObsoleteVersions = COUNT(pkID)+CASE WHEN @PublishedVersion IS NULL THEN 0 ELSE 1 END FROM tblWorkContent  WITH(NOLOCK) WHERE fkContentID=@ContentID AND HasBeenPublished=1 AND fkLanguageBranchID=@CurrentLanguage AND pkID<>@PublishedVersion
			WHILE (@ObsoleteVersions > @MaxVersions)
			BEGIN
				SELECT TOP 1 @DeleteWorkContentID=pkID FROM tblWorkContent   WITH(NOLOCK) WHERE fkContentID=@ContentID AND HasBeenPublished=1 AND fkLanguageBranchID=@CurrentLanguage AND pkID<>@PublishedVersion ORDER BY pkID ASC
...
BEGIN
	...
	SELECT	@ContentID=fkContentID,
			...
			@IsMasterLangPendingPublish = tblContent.PendingPublish
	...
	IF @IsMasterLang=0 AND @IsMasterLangPendingPublish=1
		RAISERROR (N'editPublishContentVersion: Cannot publish a language before the master language has been published (WorkID=%d)', 16, 1, @WorkContentID)
	...
	BEGIN
		...
		SET
			...
			PendingPublish  = 0,
			@FirstPublish  = tblContent.PendingPublish
		FROM 
			tblWorkContent AS W
		WHERE 
	...
	UPDATE 
		...
		SET
			...
			Created			= W.Created,
			Changed			= CASE WHEN W.ChangedOnPublish=0 AND tblContentLanguage.PendingPublish=0 THEN Changed ELSE @PublishedDate END,
			Saved			= @PublishedDate,
			...
			PendingPublish  = 0,
			PublishedVersion= @WorkContentID,
			@FirstPublish  = tblContentLanguage.PendingPublish
		FROM 
			tblWorkContent AS W
		WHERE 
...
GO
--SysRoot
INSERT INTO tblContent (pkID,fkContentTypeID,fkParentID,ArchiveContentGUID,CreatorName,ContentGUID,VisibleInMenu,Deleted,PendingPublish,ChildOrderRule,PeerOrder,ExternalFolderID, fkMasterLanguageBranchID,ContentPath, IsLeafNode) VALUES (
1,--pkID
1,--fkContentTypeID
NULL,--fkParentID
...
--SysWastebasket
INSERT INTO tblContent (pkID,fkContentTypeID,fkParentID,ArchiveContentGUID,CreatorName,ContentGUID,VisibleInMenu,Deleted,PendingPublish,ChildOrderRule,PeerOrder,ExternalFolderID,fkMasterLanguageBranchID,ContentPath) VALUES (
2,--pkID
2,--fkContentTypeID
1,--fkParentID
NULL,--ArchiveContentGUID
...
--SysGlobalAssets
INSERT INTO tblContent (pkID,fkContentTypeID,fkParentID,ArchiveContentGUID,CreatorName,ContentGUID,VisibleInMenu,Deleted,PendingPublish,ChildOrderRule,PeerOrder,ExternalFolderID,fkMasterLanguageBranchID,ContentPath,ContentType) VALUES (
3,--pkID
3,--fkContentTypeID
1,--fkParentID
NULL,--ArchiveContentGUID
...
INSERT INTO tblWorkContent (fkContentID ,fkMasterVersionID, ContentLinkGUID, fkFrameID, ArchiveContentGUID, ChangedByName, NewStatusByName, Name, URLSegment,LinkURL ,ExternalURL ,VisibleInMenu ,LinkType ,Created ,Saved ,StartPublish ,StopPublish ,ChildOrderRule ,PeerOrder ,ReadyToPublish ,ChangedOnPublish ,HasBeenPublished ,Rejected ,DelayedPublish ,RejectComment ,fkLanguageBranchID) VALUES (
1, --fkContentID
...
INSERT INTO tblContentLanguage (fkContentID,fkLanguageBranchID,PendingPublish,ContentLinkGUID,fkFrameID,CreatorName,ChangedByName,ContentGUID,Name,LinkURL,ExternalURL,AutomaticLink,FetchData,Created,Changed,Saved,StartPublish,StopPublish,PublishedVersion) VALUES (
1,--fkContentID
1,--fkLanguageBranchID
0,--PendingPublish
NULL,--ContentLinkGUID
...
'19990101 00:00',--StartPublish
NULL,--StopPublish
@fkPageId1--PublishedVersion
)

INSERT INTO tblContentLanguage (fkContentID,fkLanguageBranchID,PendingPublish,ContentLinkGUID,fkFrameID,CreatorName,ChangedByName,ContentGUID,Name,URLSegment,LinkURL,ExternalURL,AutomaticLink,FetchData,Created,Changed,Saved,StartPublish,StopPublish,PublishedVersion) VALUES (
2,--fkContentID
1,--fkLanguageBranchID
0,--PendingPublish
NULL,--ContentLinkGUID
